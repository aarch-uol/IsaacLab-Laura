        ...
        for obs_group in self.input_obs_group_shapes:
            for k in self.input_obs_group_shapes[obs_group]:
                ...
                if inputs[obs_group][k] is None:
                    continue
                # Add the prints IMMEDIATELY before the assert statement:
                # print(f"--- ROBOMIMIC DEBUG START ---")
                # print(f"  obs_group: {obs_group}")
                # print(f"  k: {k}")
                # print(f"  inputs[obs_group][k].shape: {inputs[obs_group][k].shape}")
                # print(f"  inputs[obs_group][k].ndim: {inputs[obs_group][k].ndim}")
                if obs_group in self.input_obs_group_shapes and k in self.input_obs_group_shapes[obs_group]:
                    expected_shape = self.input_obs_group_shapes[obs_group][k]
                   # print(f"  self.input_obs_group_shapes[obs_group][k]: {expected_shape}")
                    #print(f"  len(self.input_obs_group_shapes[obs_group][k]): {len(expected_shape)}")
                else:
                    # This case is very important!
                    print(f"  WARNING: self.input_obs_group_shapes does not contain key: obs_group='{obs_group}', k='{k}'")
                #print(f"--- ROBOMIMIC DEBUG END ---")
                assert inputs[obs_group][k].ndim - 2 == len(self.input_obs_group_shapes[obs_group][k])
