...
def algo_config_to_class(algo_config):
    ...
    if gaussian_enabled:
        if rnn_enabled:
            algo_class, algo_kwargs = BC_RNN_GAUSSIAN, {}
...
#### Laura Mods here 
class BC_RNN_GAUSSIAN(BC_RNN):
    """
    BC training with an RNN Gaussian policy (Heteroscedastic Uncertainty).
    """
    def _create_networks(self):
        """
        Creates networks and places them into @self.nets.
        ...
        # NOTE: This requires a new network class in robomimic.models.policy_nets
        # that implements an RNN-based policy which outputs the parameters 
        # (mean and log_std) of a Gaussian distribution for a sequence.
        self.nets["policy"] = PolicyNets.GaussianRNNActorNetwork(
            obs_shapes=self.obs_shapes,
            goal_shapes=self.goal_shapes,
            ac_dim=self.ac_dim,
            mlp_layer_dims=self.algo_config.actor_layer_dims,
    ...
    def _forward_training(self, batch):
        """
        Internal helper function to compute forward pass and return log-probabilities.
        """
        # Policy is a GaussianRNNActorNetwork that outputs a torch.Distribution object (sequence of distributions).
        dists = self.nets["policy"].forward_train(
            obs_dict=batch["obs"],  
            goal_dict=batch["goal_obs"],
        )

        # The distribution must be a batch of action sequence distributions: [B, T]
        assert len(dists.batch_shape) == 2 
        
        # Calculate log probability for the entire sequence of ground truth actions
        log_probs = dists.log_prob(batch["actions"])
    ...
    def _compute_losses(self, predictions, batch):
        """
        Loss is the Gaussian Negative Log-Likelihood (NLL).
        """
        # Loss is the negative mean log-likelihood of action targets across B x T
        action_loss = -predictions["log_probs"].mean()
        return OrderedDict(
            log_probs=-action_loss,
            action_loss=action_loss,
    ...
    def log_info(self, info):
        """
        Update log information to include NLL loss.
        """
        log = PolicyAlgo.log_info(self, info)
        log["Loss"] = info["losses"]["action_loss"].item()
        log["Log_Likelihood"] = info["losses"]["log_probs"].item()  
    ...
    def get_action(self, obs_dict, goal_dict=None):
        """
        Overrides BC_RNN.get_action(). 
        Get action using the mean of the predicted Gaussian distribution.
        """
        assert not self.nets.training
        ...
        # Use forward_step which should return the mean action and the updated RNN state
        action, self._rnn_hidden_state = self.nets["policy"].forward_step(
            obs_to_use, goal_dict=goal_dict, rnn_state=self._rnn_hidden_state)
