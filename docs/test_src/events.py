...
def reset_root_state_uniform_many(
    ...
    velocity_range: dict[str, tuple[float, float]],
    asset_cfg: SceneEntityCfg = SceneEntityCfg("robot"),
    #asset2_cfg: SceneEntityCfg = SceneEntityCfg("object"),
):
    """Reset the asset root state and some particles """
    # extract the used quantities (to enable type-hinting)
    asset: RigidObject | Articulation = env.scene[asset_cfg.name]
    #asset2: RigidObject | Articulation = env.scene[asset2_cfg.name]

    particles = []
    particlenames = [f"ball_{i}" for i in range(10)]
    for name in particlenames:
        particle = env.scene[SceneEntityCfg(name).name]
    ...
    positions = root_states[:, 0:3] + env.scene.env_origins[env_ids] + rand_samples[:, 0:3]
    offset = torch.tensor([[0.0, 0.0, 0.05]], device = asset.device)
    positions2 = root_states[:, 0:3] + env.scene.env_origins[env_ids] + rand_samples[:, 0:3] + offset
    print("random position : ", positions)
    print("particle position : ", positions2)
    orientations_delta = math_utils.quat_from_euler_xyz(rand_samples[:, 3], rand_samples[:, 4], rand_samples[:, 5])
    orientations = math_utils.quat_mul(root_states[:, 3:7], orientations_delta)
    # velocities
    range_list = [velocity_range.get(key, (0.0, 0.0)) for key in ["x", "y", "z", "roll", "pitch", "yaw"]]
    ...
    for particle in particles:
        particle.write_root_pose_to_sim(torch.cat([positions2, orientations], dim=-1), env_ids=env_ids)
        particle.write_root_velocity_to_sim(velocities, env_ids=env_ids)
