    ...
    if state == PickSmState.REST:
        ...
        if distance_below_threshold(
            ...
            if sm_wait_time[tid] >= PickSmWaitTime.LIFT_OBJECT:
                # move to next state and reset wait time
                print("[SM_INFO] : Moving from LIFT to APPROACH_OBJ2")
...
class PickAndLiftSm:
    ...
    def compute(self, ee_pose: torch.Tensor, object_pose: torch.Tensor, des_object_pose: torch.Tensor, final_object_pose: torch.Tensor) -> torch.Tensor:
       ...
       # print("loop : ", n)
        with torch.inference_mode():
           ...
           # print("debug2 : " , env.unwrapped.action_space.shape)
            ...
            final_data: RigidObjectData = env.unwrapped.scene["hot_plate"].data
