...
class PickSmState:
    ...
    POUR = wp.constant(13)
    REORIENT = wp.constant(14)
    APPROACH_GOAL = wp.constant(15)
    UNGRASP_OBJECT_2 = wp.constant(16)
...
def infer_state_machine(
    ...
    if state == PickSmState.REST:
        ...
        pose_rot = wp.transform_get_rotation(object_pose[tid])
        # Apply offset in x-direction (5 cm = 0.05 m)
        offset_pos = wp.vec3(pose_pos.x, pose_pos.y, pose_pos.z + 0.04)
        des_ee_pose[tid] = wp.transform(offset_pos, pose_rot)
        gripper_state[tid] = GripperState.OPEN
        if distance_below_threshold(
            wp.transform_get_translation(ee_pose[tid]),
        ...
        offset_pos = wp.transform_get_translation(offset[tid])
        offset_rot = wp.transform_get_rotation(offset[tid])
        offset_pos = wp.vec3(offset_pos.x, offset_pos.y + 0.15, offset_pos.z + 0.05)  # raise 25 cm ## 0.05 for sample vial
        new_offset = wp.transform(offset_pos, offset_rot)
...
class PickAndLiftSm:
    ...
    def __init__(self, dt: float, num_envs: int, device: torch.device | str = "cpu", position_threshold=0.01):
        ...
        # self.object_task2_pose_wp = wp.from_torch(self.object_task2_pose, wp.transform)
