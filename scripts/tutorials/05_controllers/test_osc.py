...
.. code-block:: bash
    ...
    # Usage
    ./isaaclab.sh -p scripts/tutorials/05_controllers/test_osc.py --num_envs 1
...
class SceneCfg(InteractiveSceneCfg):
    ...
    tilted_wall = AssetBaseCfg(
        ...
        spawn=sim_utils.CuboidCfg(
            ...
            visual_material=sim_utils.PreviewSurfaceCfg(diffuse_color=(1.0, 0.0, 0.0), opacity=0.01),
            rigid_props=sim_utils.RigidBodyPropertiesCfg(kinematic_enabled=True),
            activate_contact_sensors=True,
        ),
        init_state=AssetBaseCfg.InitialStateCfg(
            pos=(0.0, 0.0, 0.0), rot=(0.0, 0.0, 0.0, 0.0)
        ),
    )
...
def run_simulator(sim: sim_utils.SimulationContext, scene: InteractiveScene):
    ...
    ee_goal_pose_set_tilted_b = torch.tensor(
        [
            [0.4, 0.1, 0.1, 0.707, 0.0, 0.707, 0.0],
            [0.4, 0.1, 0.1, 0.5, -0.5, 0.5, -0.5],
            [0.4, 0.1, 0.3, 0.0, 0.0, 1.0, 0.0],
        ],
        device=sim.device,
    )
    ...
    #     ]
    # ], device=sim.device).repeat(scene.num_envs, 1, 1)
