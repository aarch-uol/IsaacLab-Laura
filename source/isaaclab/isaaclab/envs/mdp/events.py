...
def reset_root_state_uniform_many(
    env: ManagerBasedEnv,
    env_ids: torch.Tensor,
    pose_range: dict[str, tuple[float, float]],
    velocity_range: dict[str, tuple[float, float]],
    asset1_cfg: SceneEntityCfg = SceneEntityCfg("object1"),
    asset2_cfg: SceneEntityCfg = SceneEntityCfg("object2"),
    asset3_cfg: SceneEntityCfg = SceneEntityCfg("object3"),
):
    """Reset the asset root state to a random position and velocity uniformly within the given ranges.
    ...
    * It samples the root velocity from the given ranges and sets them into the physics simulation.
    ...
    """
    # extract the used quantities (to enable type-hinting)
    asset1: RigidObject | Articulation = env.scene[asset1_cfg.name]
    asset2: RigidObject | Articulation = env.scene[asset2_cfg.name]
    asset3: RigidObject | Articulation = env.scene[asset3_cfg.name]
    # get default root state
    root_states1 = asset1.data.default_root_state[env_ids].clone()
    root_states2 = asset2.data.default_root_state[env_ids].clone()
    root_states3 = asset3.data.default_root_state[env_ids].clone()

    # poses
    range_list = [pose_range.get(key, (0.0, 0.0)) for key in ["x", "y", "z", "roll", "pitch", "yaw"]]
    ranges = torch.tensor(range_list, device=asset1.device)
    rand_samples = math_utils.sample_uniform(ranges[:, 0], ranges[:, 1], (len(env_ids), 6), device=asset1.device)
    offset = torch.tensor([[0.0, 0.2, 0.0]], device = asset1.device)

    positions1 = root_states1[:, 0:3] + env.scene.env_origins[env_ids] + rand_samples[:, 0:3]
    positions2 = root_states2[:, 0:3] + env.scene.env_origins[env_ids] + rand_samples[:, 0:3] - offset
    positions3 = root_states3[:, 0:3] + env.scene.env_origins[env_ids] + rand_samples[:, 0:3] + offset
    
    orientations_delta = math_utils.quat_from_euler_xyz(rand_samples[:, 3], rand_samples[:, 4], rand_samples[:, 5])
    orientations = math_utils.quat_mul(root_states1[:, 3:7], orientations_delta)
    # velocities
    range_list = [velocity_range.get(key, (0.0, 0.0)) for key in ["x", "y", "z", "roll", "pitch", "yaw"]]
    ranges = torch.tensor(range_list, device=asset1.device)
    rand_samples = math_utils.sample_uniform(ranges[:, 0], ranges[:, 1], (len(env_ids), 6), device=asset1.device)

    velocities = root_states1[:, 7:13] + rand_samples

    # set into the physics simulation
    asset1.write_root_pose_to_sim(torch.cat([positions1, orientations], dim=-1), env_ids=env_ids)
    asset1.write_root_velocity_to_sim(velocities, env_ids=env_ids)

    asset2.write_root_pose_to_sim(torch.cat([positions2, orientations], dim=-1), env_ids=env_ids)
    asset2.write_root_velocity_to_sim(velocities, env_ids=env_ids)

    asset3.write_root_pose_to_sim(torch.cat([positions3, orientations], dim=-1), env_ids=env_ids)
    asset3.write_root_velocity_to_sim(velocities, env_ids=env_ids)
...
def reset_scene_to_default(env: ManagerBasedEnv, env_ids: torch.Tensor):
    ...
    print("[RESET]  : Rest scene to default")
    # rigid bodies
